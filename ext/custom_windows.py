...
class PathConfig(tkinter.simpledialog.Dialog):
    def body(self, master):
        # Load config
        with open(loc+'\\data.json') as json_file:
        ...
        self.minsize(width=250, height=100)
        Label(master, text="Game Executable Location").grid(column=0, row=0, sticky=NW)
        self.game_exe_location = StringVar(value=data["exe"] if "exe" in data else "")
        self.game_exe_entry = Entry(master, textvariable=self.game_exe_location, width=40)
        self.game_exe_entry.grid(row=1,column=0, columnspan=2,sticky=W, pady=5)
        self.game_exe_button = Button(master, text="Select", command=lambda: self.game_exe_location.set(askopenfilename(initialdir="C:\\Program Files (x86)\\Steam\\steamapps\\common\\DELTARUNEdemo", filetypes=[("Executable", "*.exe")])))
        self.game_exe_button.grid(row=1, column=2, sticky=W)

        Label(master, text="Game Data Location").grid(column=0, row=2, sticky=NW)
        if data["data"] == "": data["data"] = os.getenv("LOCALAPPDATA")+"\\DELTARUNE"
        self.game_data_location = StringVar(value=data["data"] if "data" in data else "")
        self.game_data_entry = Entry(master, textvariable=self.game_data_location, width=40)
        self.game_data_entry.grid(row=3,column=0, columnspan=2,sticky=W, pady=5)
        self.game_data_button = Button(master, text="Select", command=lambda: self.game_data_location.set(askdirectory(initialdir=data["data"])))
        self.game_data_button.grid(row=3, column=2, sticky=W)


        
        return

    def select_file(self, default):
        self.fileName = askopenfilename(initialdir="C:\\Program Files (x86)\\Steam\\steamapps\\common\\DELTARUNEdemo", filetypes=[("Executable", "*.exe")])
        self.file_string_truncated.set("..."+self.fileName[-32:])
    ...
    def apply(self):
        with open(loc+'\\data.json') as json_file:
            data = json.load(json_file)
        data["exe"] = self.game_exe_location.get()
        data["data"] = self.game_data_location.get()
...
class Settings(tkinter.simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        Button(master, text="Open Deltarune Saves Folder", command=lambda: os.system(f"explorer.exe {vars['data']}")).grid(row=1)
        Button(master, text="Change File Paths", command=lambda: PathConfig(self)).grid(row=2)
        return
    def buttonbox(self):
        box = Frame(self)
